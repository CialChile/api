...
class RolesAdminController extends Controller
{
    ...
    {
        $this->module = 'admin-security-roles';
        $this->roleRepository = $roleRepository;
    }

    public function index()
    {
        $this->userCan('list.admin-security-users|list');
    ...
    {
        $this->userCan('list');
        return Datatables::of(Role::where('company_id', 0))
    ...
    public function update(RoleUpdateRequest $request, $roleId)
    {
        ...
        $user = $this->loggedInUser();
        $permissions = collect($data['permissions']);
        $roles = $this->roleRepository->findWhere(['id' => $roleId, 'company_id' => 0]);
