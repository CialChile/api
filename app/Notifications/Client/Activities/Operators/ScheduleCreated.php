...
class ScheduleCreated extends Notification
{
    ...
    {
        $user = $this->scheduleExecution->activitySchedule->activity->createdBy;
        list($activityName, $data) = $this->getMessageData($user);
        $data['body'] = 'Le asigno una nueva actividad ' . $activityName . ' para su ejecución el día ' . $data['execution_date'];
        return $data;
    }
}
